...
TMP006::TMP006(uint8_t address, uint16_t sample_rate, uint8_t mode)
    : TWI::Driver(address),
...
float TMP006::read_value() {
    switch (m_mode) {
        case DIE:
            return static_cast<float>(read_die_temperature());
        case OBJECT:
            return static_cast<float>(read_obj_temperature());
        default:
            return 0.0f;
    }
}
